//grunt-start
/// <reference path="../Options.ts" />
/// <reference path="../Parser.ts" />
/// <reference path="../ParserList.ts" />
/// <reference path="../ParserManager.ts" />
/// <reference path="../RegExpUtils.ts" />
/// <reference path="../TranslationEntry.ts" />
/// <reference path="../Utils.ts" />
/// <reference path="gruntjs/gruntjs.d.ts" />
/// <reference path="node/node.d.ts" />
/// <reference path="../parsers/AngularTranslateParser.ts" />
/// <reference path="../parsers/PHPGettextParser.ts" />
/// <reference path="../parsers/WordpressParser.ts" />
/// <reference path="../translate_extract.ts" />
/// <reference path="../../test/test.ts" />
//grunt-end